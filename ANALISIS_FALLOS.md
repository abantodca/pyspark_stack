...
> **Estado:** todos los fixes (#1–#6) aplicados el 2026-07-12, más la limpieza del `docker-compose.yml` (#7)
> y el fallo recurrente de infraestructura (#8, `docker-proxy` huérfanos).
> Último reset limpio (2026-07-12): `down -v` + `build --no-cache` + `up -d` → los 10 contenedores `running`
> (más `airflow-init` en `exited (0)`; `airflow-db` `healthy` —único servicio con healthcheck—, worker Spark
> registrado, 3 DAGs sin errores de import).
>
> **Migración a Airflow 3.2.2 (2026-07-12):** se actualizó Airflow de `2.7.2` a `3.2.2` (rama 3.2, el parche
> más maduro; se descartó `3.3.0` por tener solo 6 días de rodaje). Ver §5. Cambia la topología: ahora hay
> api-server + scheduler + dag-processor + triggerer + init en vez de webserver + scheduler.
> Requiere `down -v` + `build --no-cache` (imagen y esquema de BD nuevos).
>
> **Stack "lo más actual" + verificación end-to-end (2026-07-12):** para mantener Python 3.12 (lo trae la
> imagen de Airflow 3) hubo que subir todo el cluster Spark de 3.2.1 a `4.0.3`, ya que Python 3.11+ es
> incompatible con el cloudpickle de PySpark 3.2.x. Spark 4 exige Java 17. Además `postgres 13 → 16`.
> Los 3 DAGs quedaron verificados en `success` con salida real. Ver §5.3 (efecto dominó) y §6
> (cloudpickle, imagen vieja, `PYTHON_VERSION_MISMATCH`, permisos HDFS, `Wrong FS`, des-sincronización del bind-mount).
>
...
> Ver §5 para el detalle de rupturas y comandos.
...
Los directorios `./spark-apps`, `./dags` y `./spark-events` se montan como volúmenes dentro de los contenedores.
...
> Nota: el stack migró después a Spark 4.0.3, cuyo comportamiento por defecto es el mismo (`multiline=false`), así que el diagnóstico sigue válido.

**Síntoma:** el `JOIN ... ON o.product_id = p.product_id` (o `s.product_id = p.product_id`) revienta con
`AnalysisException: cannot resolve 'p.product_id' given input columns: [_corrupt_record]`.

> `orders.json` (en `spark-apps/input/orders.json`) sí está en JSONL; por eso `order_enrichment_job.py` y `practice_script.py` funcionan. El problema es exclusivo de los `products.json`.
...
Recomendada la opción A: menos frágil, no depende de cómo se regenere el archivo.
...
**Fix:** crear los scripts, o eliminar/deshabilitar esos DAGs si eran solo de prueba.

> **Aplicado:** se crearon `spark-apps/wordcount.py` y `spark-apps/wordcount_hdfs.py`, ambos self-contained:
> generan su propio texto de entrada y no dependen de archivos previos en el FS/HDFS. La variante HDFS
...
### 🟠 MEDIO 3 — Ruta WSL hardcodeada en la rama "fuera de contenedor"  ✅ APLICADO
...
**Impacto:** solo afecta si el script se ejecuta fuera del contenedor. Airflow lo corre dentro (rama `[ -f /.dockerenv ]`), que usa la ruta correcta `/opt/spark-apps/...`; en el flujo normal no rompe, pero falla si alguien lo lanza desde el host.
...
> **Aplicado:** `customer_etl_job_airflow.sh:74` ahora usa `source "$(dirname "$0")/../config/env.sh" "$ENV"`,
...
**Causa:** `spark-defaults.conf` tiene `spark.eventLog.enabled true` con `dir file:/tmp/spark-events`, pero el History Server está comentado. Además hay dos ficheros `*.inprogress` viejos (`app-2025051*`) de corridas que nunca cerraron su log.

**Impacto:** no rompe los jobs, pero:
- No hay UI de historial (puerto 18080) pese a generarse eventos.
- Si `spark-defaults.conf` llega a montarse en Spark, `eventLog` escribe en `/tmp/spark-events`; si ese dir no existe o no tiene permisos, el job puede fallar al iniciar.

**Fix:** reactivar el servicio `spark-history-server`, o poner `spark.eventLog.enabled false` para evitar logs huérfanos.

> **Aplicado:** se puso `spark.eventLog.enabled false` en `spark-defaults.conf` (con comentario sobre cómo
...
**Causa:** 2026 no es bisiesto; el 29 de febrero no existe. Cosmético, pero denota fecha escrita a mano sin validar.

**Fix:** corregir a una fecha real.
...
**Causa:** las variables (`ENV`, `HDFS_INPUT`, `HDFS_OUTPUT`, `FINAL_CSV`, `RUN_DATE`, `LANDING_PATH`) solo quedan disponibles porque el shell hace `source env.sh` en el mismo proceso. Cualquier subproceso o refactor que llame a `env.sh` sin `source` perderá los valores silenciosamente.

**Impacto:** ninguno en el flujo actual; riesgo latente.

**Fix:** descomentar los `export`.

> **Aplicado:** descomentados los 6 `export` en `env.sh`.

---

### 🧹 MENOR 7 — `version: "3.8"` obsoleto en `docker-compose.yml`  ✅ APLICADO

**Dónde:** `docker-compose.yml:1` (clave `version: "3.8"` de cabecera).

**Causa/Impacto:** Compose v2+ ignora la clave `version` y emite un warning en cada comando; es ruido sin efecto funcional.

> **Aplicado:** se eliminó la línea `version: "3.8"` del `docker-compose.yml`.
...
**Síntoma:** tras `docker compose up -d`, varios contenedores quedan en estado `Created` y no arrancan. Solo suben los que no publican puertos conflictivos (p. ej. `hdfs-namenode`, `airflow-db`). Al intentar `docker start spark-master`:
...
> y los 10 contenedores quedaron `running` (más `airflow-init` en `exited (0)`; `airflow-db` `healthy` —único
> servicio con healthcheck—, worker registrado con el master, 3 DAGs cargados sin errores de import).
>
> **Prevención:** ante contenedores atascados en `Created` con `address already in use`, revisar siempre
> `pgrep -a docker-proxy` antes que el código o el compose.
...
| 2 | 🔴 Crítico | ✅ | `dags/spark_trigger_dag.py`, `dags/spark_trigger_hdfs_dag.py` | Creados `wordcount.py`/`wordcount_hdfs.py` (self-contained) | Apuntaban a scripts inexistentes → tarea `failed` |
| 3 | 🟠 Medio | ✅ | `customer_etl_job_airflow.sh:74` | Ruta relativa `$(dirname "$0")/../config/env.sh` en vez de `/mnt/c/...` | Ruta WSL hardcodeada rompía fuera del contenedor |
| 4 | 🟠 Medio | ✅ | `spark-defaults.conf`, `spark-events/*.inprogress` | `eventLog.enabled false` + borrados los `.inprogress` huérfanos | Logs huérfanos sin UI que los consuma |
...
## 4. Orden que se siguió para dejarlo funcionando (histórico — ya aplicado)
...
3. Ajustar la ruta del shell (Medio 3) si se ejecuta fuera del contenedor.
...
| Por qué 3.2.2 y no 3.3.0 | 3.3.0 salió el 6-jul-2026, con 6 días de rodaje. 3.2.2 (29-may-2026) es el último parche de la rama 3.2: más maduro y sin sorpresas de un `.0` recién publicado. |
| Otras candidatas | 3.1.8 (11-mar-2026, más conservadora) · 3.3.0 (la última "latest stable" según los docs). |

> Airflow 2.x quedó EOL (soporte terminó el 22-oct-2025); quedarse en 2.7.2 ya no recibe parches de seguridad.
...
| Conexión SQLAlchemy | `AIRFLOW__CORE__SQL_ALCHEMY_CONN` | `AIRFLOW__DATABASE__SQL_ALCHEMY_CONN` (sección `[database]`) |
...
Mantener Python 3.12 (lo trae `apache/airflow:3.2.2-python3.12`) obligó a subir todo el stack Spark. La cadena de decisiones:
...
- **Se eligió Spark 4.0.3.** Spark 4 requiere Java 17 (ya no soporta Java 11), así que en
...
La config repetida se factorizó en una ancla YAML `x-airflow-common` (build + env + volúmenes + red). Los servicios usan `depends_on` con `condition: service_healthy` (postgres) y `service_completed_successfully` (init): el arranque es ordenado y ya no hacen falta los bucles `while ! pg_isready ...` del compose anterior.

### 5.6 Cómo aplicar la actualización

Cambian la imagen y el esquema de la BD, así que hay que resetear (el volumen `postgres_data` traía metadata de Airflow 2):
...
No aplicaba a estos DAGs, pero a tener en cuenta si crecen: contexto eliminado (`execution_date`, `tomorrow_ds` → usar `logical_date` / `data_interval_*`), sin acceso directo a la BD de metadata desde las tasks, y SubDAGs / SequentialExecutor eliminados. Ayuda: `airflow config update --fix` dentro del contenedor y la guía oficial *Upgrading to Airflow 3*. Tras el primer arranque, `docker compose logs airflow-dag-processor` muestra errores de parseo si los hubiera.
...
1. **Imagen vieja de Airflow reutilizada** (scheduler en crash-loop "upgrade the database... 2.7.2").
   ...
   `image: pyspark_stack-airflow:3.2.2`, así los 5 servicios comparten una sola imagen construida una vez. Se
   borraron las imágenes `pyspark_stack-airflow-*` viejas y se reconstruyó.
...
3. **`PicklingError: IndexError: tuple index out of range` (cloudpickle) al ejecutar el wordcount.** PySpark 3.2.1
   no serializa lambdas en Python 3.12. Fix de fondo: subir todo Spark a 4.0.3 (ver §5.3), lo que arrastró
...
9. **Bind-mount que no refleja los edits (quirk del entorno).** El mount de `./spark-apps` es de tipo
   ...
   inodo distinto entre host y contenedor. Workaround: forzar una escritura *in-place* (append/truncate
   preservando inodo) para re-sincronizar. Los `.py` de `dags/` sí se re-sincronizaron vía
...
> ⚠️ **Falsos positivos:** `customer_etl_job_airflow.sh` no usa `set -e`, así que devuelve el código del último
> `echo` y el DAG queda en `success` aunque el `hdfs put` o el `spark-submit` intermedios hayan fallado.
> Validar siempre la salida real (CSV no vacío / `hdfs dfs -cat` del output), no solo el estado del DAG.
