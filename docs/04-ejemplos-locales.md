...
observar** y el **por qué**, con las trampas propias de este montaje.

Los datos son los reales del repositorio: `spark-apps/landing/customer_etl/`.

**Requisito:** el stack arriba (`docker compose up -d`). UIs: Airflow `:8082`, Jupyter `:8888`,
Spark `:8081`, HDFS `:9870`.

**Esquema de los datos** (se usa en casi todos los ejemplos):
...
> Si un contenedor queda en `Created` con `address already in use`, identificá primero el proceso
...
> Regla: `stop`/`start` para el día a día · `down` para liberar contenedores conservando los datos ·
> `down -v` **solo** para empezar limpio o tras cambiar un `Dockerfile`. Usalo únicamente cuando
> perder HDFS y la metadata de Airflow sea intencional.

### 0.7 Por dónde empezar

Con el stack verificado (0.2), empezá por el **Ejemplo 1** en Jupyter (`:8888`) y avanzá en orden.
Cada ejemplo se apoya en el anterior; no saltees el 6 (HDFS) ni el 7 (JSON multilínea), que
desbloquean los avanzados.
...
**Por qué:** confirmás que el notebook-driver **se conecta al cluster real**, no a un Spark local.
...
**Dónde:** terminal del host. Creá el script y lanzalo como se hace en producción.
...
**Por qué:** este es el patrón real de ejecución, el mismo que usan los DAGs. Fijate que acá el
...
**Qué observar:** probá a quitar `.option("multiline","true")` y leer `products.json`: vas a ver una
...
**Qué observar:** DataFrame API y SQL producen el **mismo plan** (comprobalo con `.explain()`).
Elegí el estilo que quede más claro; se pueden mezclar.
...
```
El Ej. 6 solo subió `orders.csv`. Este job lee las tres fuentes, así que **primero completá la
landing en HDFS** — si lo salteás, falla con `Path does not exist: /ejemplos/input/customers.csv`:

```bash
docker exec -e HADOOP_USER_NAME=root hdfs-namenode \
  hdfs dfs -put -f /opt/spark-apps/landing/customer_etl/customers.csv  /ejemplos/input/
docker exec -e HADOOP_USER_NAME=root hdfs-namenode \
  hdfs dfs -put -f /opt/spark-apps/landing/customer_etl/products.json  /ejemplos/input/
docker exec hdfs-namenode hdfs dfs -ls /ejemplos/input   # deben aparecer los 3 archivos
```
...
`customer_etl_dag`. En prod ese Bash se cambia por un operador de EMR Serverless. Este DAG ejecuta
...
el agregado. El `checkpointLocation` recuerda qué archivos ya procesó → si reiniciás, **no** los
reprocesa.
...
(`:8081` → tu app → pestaña **SQL**): vas a ver el DAG con las particiones que AQE **coalescó** en runtime.
...
([guía 02 §16](02-produccion-aws-terraform.md#16-athena-e-iceberg)),
...
[guía 02 §6.4](02-produccion-aws-terraform.md#64-cómputo-spark-emr-serverless). Practicar el `MERGE`/time-travel
...
|---|---|---|
| Cómputo Spark (EMR Serverless) | Ej. 1–20 son el mismo Spark; solo cambia el `--master`/submit | [docs/02 §6.4](02-produccion-aws-terraform.md#64-cómputo-spark-emr-serverless) |
...
Ver [docs/02](02-produccion-aws-terraform.md) (el cómo, copy-paste) y [docs/03](03-arquitectura.md) (el mapa).
