...
8. [Refactorización profesional (versión endurecida)](#8-refactorización-profesional)
  ...
  airflow-init:          # one-shot: migra esquema + crea admin, luego termina
    ...
    command: >
      bash -c "
        ...
        airflow users create --username admin ... --password admin || true"
...
services:
  ...
  # Los 5 airflow-* ya tienen restart: always; solo añadimos logging + límites
