...
| OpenLineage | EC2 (Airflow + dbt) | Lineage dataset-a-dataset; eventos JSON a S3, consultables por Athena (guía 02 §21) |
...
| Lambda `trigger-airflow` (+ DLQ) | AWS | Dispara DAGs vía SSM, con contrato de datos y auto-start de la EC2 si está apagada; DLQ compartida con la cola de arriba (guía 02 §7.1/§18.1) |
| Lambda `startstop` (+ DLQ) | AWS | Prende y apaga la EC2; SQS como dead-letter si falla (guía 02 §18.1) |
...
> (~30 min), cae a la misma DLQ que ya tenían las Lambdas (guía 02 §18.1), con alarma → email. La
...
> [guía 02 §9.1](02-produccion-aws.md#91-instalar-los-providers-de-airflow-amazon-papermill-y-sus-dependencias).
...
**Cómo queda montado** (detalle completo, Terraform y SQL: [guía 02 §16.1](02-produccion-aws.md#161-tablas-iceberg-acid-time-travel-y-merge-desde-sql-sin-crawler) y [§6.4](02-produccion-aws.md#64-cómputo-spark-emr-serverless)):
...
[guía 02 §16.1b](02-produccion-aws.md#161b-mantenimiento-compactación-y-expiración-de-snapshots-auditoría-21).
...
[guía 02](02-produccion-aws.md).

- **dbt Core** — transformaciones SQL versionadas `curated → analytics`, con **dos targets**: `athena`
  (SQL directo sobre las tablas Iceberg, sin cómputo Spark — para transforms livianos/frecuentes) y
  `spark` (vía EMR Serverless, para reprocesos pesados). El DAG elige el target por task, mismo
  criterio que la tabla "¿PySpark o Python puro?" de la guía 02 §9.0. Reemplaza SQL que hoy viviría
  hardcodeado dentro de un DAG por modelos versionados, testeados y documentados en Git.
...
[guía 02 §18](02-produccion-aws.md#18-gobierno-costo-y-resiliencia-extras):

| Pieza | Qué resuelve |
|---|---|
| **DLQ (SQS) en `trigger-airflow` y `startstop`** | Sin esto, un evento `S3 ObjectCreated` que dispara una Lambda fallida se perdía en silencio — nadie se enteraba hasta el día siguiente (o nunca, si era event-driven) |
