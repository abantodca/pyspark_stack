...
Arquitectura híbrida de bajo costo: Airflow y Postgres corren en una EC2 chica, Spark corre en EMR
Serverless, S3 es el data lake, y EventBridge, SQS, Lambda y SSM automatizan los disparos. Terraform
administra la infraestructura y GitHub Actions entrega el código con OIDC, sin claves permanentes.

> **Estado: guía completa, sin desplegar.** Ninguna sección fue validada de extremo a extremo en
> AWS. Observabilidad, Iceberg, dbt, Great Expectations, OpenLineage y CD son arquitectura objetivo.
> Consultá la [matriz de estado](README.md) antes de ejecutar una sección.
>
> El diseño es *single-node* para el plano de control: prioriza simplicidad y costo por encima de
> alta disponibilidad. EMR Serverless escala el cómputo, pero la EC2 sigue siendo un punto único de
> fallo para Airflow, Postgres y el monitoreo.

## Cómo usar esta guía

- El repositorio versiona **solo el proyecto local**. Todo lo de producción —Terraform, Compose de
  producción, Lambdas, DAGs de EMR, workflows de CI/CD— se crea siguiendo esta guía, que trae el
  contenido íntegro de cada archivo. Los bloques de código son la fuente: copialos tal cual.
...
EC2 (costo $0 adicional). Para el entorno de desarrollo local, ver `docs/01-stack-local.md`.
...
> No hace falta crear toda esta estructura al inicio: es el resultado final. La guía agrega cada
> archivo cuando aparece por primera vez e indica después cuándo ampliar uno existente. `bootstrap`
> se ejecuta una sola vez; todas las secciones posteriores agregan recursos dentro de `infra/prod`,
> con **un solo módulo raíz y un solo state** — los nombres de archivo describen responsabilidades,
> no directorios Terraform independientes.

### Dos formas de crear la infra: Terraform y consola

Los recursos de las secciones 4 a 7 —las que construyen la plataforma— traen su Terraform
(copy-paste) y, debajo, un desplegable «🖱️ A mano en la consola AWS» con los pasos equivalentes,
para entender qué crea cada bloque. De la sección 8 en adelante la guía es operativa y solo usa
CLI. Si necesitás el camino manual completo, está en la [guía 02b](02b-produccion-aws-consola.md).
...
> **Sobre `apply` sin `plan -out`.** Durante la construcción inicial (§4–§7) la guía usa `apply` a
> secas: es interactivo, muestra el plan y pide confirmación, y el entorno todavía no tiene datos que
> perder. Esa comodidad deja de aplicar en cuanto el stack está en producción: a partir de ahí, todo
> cambio pasa por `plan -out=tfplan`, revisión del plan guardado y `apply tfplan` (§15 y §21.2). El
> [checklist de readiness](05-production-readiness.md) exige esa evidencia antes del primer
> despliegue.
...
> else
>   printf 'my_ip_cidr     = "%s/32"\n' "$MY_IP" >> infra/prod/terraform.tfvars
> fi
> if ! grep -q '^ssh_public_key' infra/prod/terraform.tfvars 2>/dev/null; then
>   printf 'ssh_public_key = "%s"\n' "$(cat ~/.ssh/pyspark_stack.pub)" >> infra/prod/terraform.tfvars
> fi
> terraform -chdir=infra/prod apply
> ```
>
> Sin setup extra ni permisos IAM nuevos. Es la opción que conviene si tu IP cambia cada tantos días.
>
> El bloque usa `printf >>` además del `sed` a propósito: `sed -i` solo reemplaza líneas que ya
> existen y, si el patrón no matchea, **no avisa nada** — parece que corrió bien y no cambió un
> carácter. Lo mismo con `ssh_public_key`, que no tiene default (§5.2): si falta en `tfvars`, `apply`
> la pide por prompt y un Enter en blanco manda `""` a AWS, que falla más tarde con
> `Error: importing EC2 Key Pair: MissingParameter: PublicKeyMaterial`. La `validation` de
> `variables.tf` corta ese caso en seco.
>
> ⚠️ **No uses `-var "my_ip_cidr=..."` sin tocar `tfvars`.** El flag vale solo para ese comando: el
> archivo sigue con el valor viejo. De acá en adelante todas las secciones te hacen correr
> `terraform apply` a secas, y ese apply reaplica lo que hay en `tfvars`, revirtiendo el security
> group a la IP vieja y **cortándote el SSH que tenías andando**. Es la causa más común de «se me cae
> el SSH cada vez que sigo con la guía».
>
...
Esta es la versión **mínima**: Airflow + Postgres, lo justo para que §5.5 termine con un stack en
pie. La versión definitiva —variables de EMR Serverless, secretos desde SSM, límites de memoria y
métricas— se entrega completa y una sola vez en **§14.1**, y ahí reemplazás el archivo entero. No lo
vayas parcheando por partes a medida que avanzás:
...
services:
  ...
  airflow-apiserver:
    ...
    command: api-server
    ports:
      - 127.0.0.1:8082:8080   # solo túnel SSH; §5.6 agrega 443 si exponés la web
    depends_on:
      airflow-db: { condition: service_healthy }
...
> acá— el comando para levantar el stack pasa a ser (§14.1):
...
Esta sección es **opcional**. Al terminar §5.5, Airflow continúa disponible únicamente mediante el
túnel SSH a `localhost:8082`; no necesitás crear ningún archivo HTTPS si ese acceso te alcanza.

Si querés entrar sin túnel, seguí los pasos de esta sección **en orden**. Al final se crea
`docker-compose.prod.https.yml`, un override pequeño que agrega TLS y el puerto 443 sin modificar el
compose base. Esto mantiene dos modos explícitos:
...
No crees el override antes de emitir el certificado: contiene variables obligatorias y rutas que
todavía no existen. Esta sección publica **solo la web de Airflow** por HTTPS, y el security group
permite el puerto 443 únicamente desde `var.my_ip_cidr`. El resto de las UIs continúa por túnel.
...
`AIRFLOW_SSL_KEY`. Podés generar sus valores localmente para inspeccionarlos:
...
No hace falta escribirlas en el `.env` local: `rsync` lo excluye y Docker Compose debe leerlas en
la EC2. Cargalas allí por uno de estos dos caminos:
...
El override conserva el puerto `8082` y los volúmenes del archivo base, y agrega el `443`, el
montaje de certificados y el alias interno. Se monta `/data/certs` completo porque los archivos de
`live/` son enlaces relativos hacia `archive/`. Las expresiones `:?` hacen que `docker compose`
...
data "aws_iam_policy_document" "trigger_airflow" {
  ...
  # ahora haría fallar el `apply` de esta sección con "Reference to undeclared resource". §7.3 te
  # hace agregar ese statement junto con la cola.
  statement {
    actions   = ["logs:CreateLogGroup", "logs:CreateLogStream", "logs:PutLogEvents"]
    resources = ["arn:aws:logs:*:*:*"]
  }
...
**Agregá ahora el permiso de consumo** al `aws_iam_policy_document "trigger_airflow"` de §7.1 — es el
statement que se dejó afuera allá porque la cola todavía no existía. Sin él, el event source mapping
queda creado pero en estado `Disabled` y los mensajes se acumulan sin que nadie los lea:

```hcl
# infra/prod/orchestration.tf — dentro de data "aws_iam_policy_document" "trigger_airflow" (§7.1)
  statement {
...
Detené en la primera capa que falle. Un DAG no se puede diagnosticar correctamente si SSM está
...
> **Prerrequisito.** Esta prueba dispara el DAG `customer_etl_emr`, que se escribe en **§9.4** y
> llega a la EC2 con el deploy de §10 u §11. Si todavía no existe, la Lambda responde 200 —eso solo
> confirma que atendió la invocación— y el comando SSM termina con `DAG customer_etl_emr not found`.
> Es lo esperado: volvé a esta sección después de §9.4.
...
`commandId`, comprobá el resultado real:
...
Este fragmento enumera únicamente las dependencias adicionales de producción; se suman a los
providers que ya usa el stack local, no los reemplazan.
...
Creá el archivo con este contenido. Al agregarlo, ampliá `tests/test_dag_integrity.py` para que
cubra también este DAG (§11.3).
...
lo que queda en la EC2 puede no coincidir con `main`. Producción se despliega con el workflow de
§11.4, que es el único camino auditable.
...
- El rol escribe únicamente en el bucket de artifacts y ejecutá SSM sobre la EC2 prevista.
...
El job de DAGs instala **`apache-airflow==3.2.2` con el mismo constraints file que
`Dockerfile.airflow.prod`** (§5.5, Paso 0b). Es el punto que hace que el CI sirva de algo: un
`pip install -r requirements.txt` a secas deja que pip resuelva la versión de Airflow por su cuenta,
y el `DagBag` termina validándose contra una versión distinta de la que corre en la EC2 — con lo cual
un DAG puede pasar el CI y romper en producción por un cambio de API entre minors.
...
jobs:
  ...
  terraform:
    ...
    steps:
      ...
      - run: terraform -chdir=infra/prod init -backend=false
...
**Archivo:** `tests/test_dag_integrity.py` — el repositorio ya trae la versión local; agregale el
contrato del DAG de producción creado en §9.4.
...
> **Roadmap:** los archivos `monitoring/` de esta sección todavía no están versionados. Define
> criterios de implementación y no forma parte aún del runbook ejecutable. El Compose que levanta
> estos servicios es el override de [§14.2](#142-docker-composeprodmonitoringyml--override-de-observabilidad);
> hasta que `monitoring/` exista, se corre solo el archivo base.
...
Validá el nombre real de la métrica del scheduler en StatsD exporter antes de activar la segunda
...
1. Leé el log de la task.
2. Identificá si falló Airflow o el servicio remoto.
3. Si existe job ID de EMR, revisá `stateDetails` y logs.
4. Corregí la causa; después limpiá o reintentá la task.

**Evento no procesado**

1. Comprobá que el objeto cumple prefijo y sufijo.
2. Revisá mensajes visibles y no visibles en SQS.
3. Revisá errores y throttles de Lambda.
4. Revisá la DLQ.
5. Reprocesá con el mismo `bucket`, `key` y `sequencer`.

**EC2 sin espacio**

1. No borres `/data/postgres`.
2. Revisá logs Docker, Loki y Prometheus.
3. Aplicá retención o ampliá el EBS.
4. Creá snapshot antes de una modificación de volumen.
...
- Usá roles distintos para EC2, EMR job, Lambda y GitHub.
- Restringí SSM al ARN de la instancia y al documento requerido.
- Exigí IMDSv2.
- Mantené S3 privado, cifrado y con política `aws:SecureTransport`.
- Enlazá UIs a loopback o limita 443 a un `/32`.
- Registrá CloudTrail y revisá Access Analyzer.
...
No imprimas los valores ni uses `set -x`. Si elegís crear `SecureString` con Terraform, recordá
...
**Dónde vive y cómo llega.** El script se escribe en tu repo local, pero **se ejecuta en la EC2**:
necesita el instance profile para leer los parámetros SSM. Corrido en tu máquina, o falla por
permisos o —peor— te deja los secretos de producción en un `.env` local. Como se creó después del
`rsync` de §5.5, hay que subirlo:
...
- mantené imágenes pineadas;
- eliminalos si CloudWatch Container Insights cubre lo que necesitás.

---

## 14. Compose canónico de producción

Esta sección reemplaza por completo el `docker-compose.prod.yml` mínimo del Paso 0 (§5.5). Spark,
HDFS y Jupyter no se ejecutan en la EC2.

Se divide en **dos archivos**, con el mismo criterio que el override HTTPS de §5.6:
...
La separación no es cosmética: los servicios de observabilidad montan archivos de `./monitoring/**`
que todavía no existen en el repositorio. En un único archivo, un `docker compose up` los tomaría
como bind mounts a rutas inexistentes —Docker las crea como directorios vacíos propiedad de root— y
Prometheus, Grafana y Loki arrancarían en crash-loop con un error de configuración que no dice cuál
es la causa real. Manteniéndolos en un override, el stack base arranca limpio y el monitoreo se suma
recién cuando su configuración existe y valida.
...
x-airflow-common: &airflow-common
  ...
  environment: &airflow-env
    ...
    # así que no rompe nada, pero tampoco sirve de nada. Ponelo en True junto con el override.
...
services:
  ...
  airflow-scheduler:
    ...
    container_name: airflow-scheduler
    command: scheduler
    # Cuando incorpores dbt (§19) o Great Expectations (§20), agregá acá sus montajes:
    #   - ./dbt:/opt/dbt:ro
...
**Arranque del stack base.** Este archivo reemplaza al mínimo de §5.5, así que primero subí la
versión nueva a la EC2 y recién después levantá. Los dos primeros comandos corren **en tu máquina**;
el resto, **en la EC2**:
...
`up -d --build` reconstruye la imagen porque cambió el Compose, no el Dockerfile; si además tocaste
`requirements.txt` o `Dockerfile.airflow.prod`, agregá `--no-cache` para que no reutilice capas
viejas de pip.

### 14.2 `docker-compose.prod.monitoring.yml` — override de observabilidad

> **Roadmap.** Aplicá este override recién cuando `monitoring/` exista en el repositorio con las
...
Arranque con observabilidad — los dos archivos, siempre juntos y en este orden:
...
Las UIs quedan enlazadas a `127.0.0.1` y se acceden por túnel SSH (§12.3); ninguna se publica en el
security group.
...
Ejecutá las secciones 8.2 y 8.3. La promoción termina solo cuando:
...
Guardá el commit, el `terraform plan`, el DAG run ID, el EMR job ID y el resultado del smoke test.
...
Ejecutá mantenimiento fuera de la ventana ETL:
...
Usalo para APIs, archivos pequeños y tareas de control:
...
tenían. Creá la cola, dale permiso al rol del Scheduler para escribirle y recién después agregá el
`dead_letter_config` a los `target` de cada `aws_scheduler_schedule`:
...
Con la cola ya declarada, agregá este bloque dentro del `target` de cada `aws_scheduler_schedule`
(§5.4 `start`/`stop` y §7.2 `etl_cron`):
...
2. Revisá el plan y el diff.
3. Despliega desde un solo canal.
4. Ejecutá smoke test y una corrida controlada.
5. Promueve o revierte.

### 21.3 Recuperación

Probá trimestralmente:
...
Automatizarlo en un script solo tiene sentido en entornos descartables. En producción, eliminar EBS,
buckets y backend exige una revisión aparte y un respaldo verificado.
